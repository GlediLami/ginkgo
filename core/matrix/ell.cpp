...
template <typename ValueType, typename IndexType>
void Ell<ValueType, IndexType>::apply_impl(const IMultiVector* b,
                                           IMultiVector* x) const
{
    apply_mixed_precision_dispatch<ValueType>(
        [this](auto view_b, auto view_x, auto...) {
            this->get_executor()->run(
                ell::make_spmv(this->get_const_device_view(), view_b, view_x));
        },
        b, x);
}
...
template <typename ValueType, typename IndexType>
void Ell<ValueType, IndexType>::apply_impl(const IMultiVector* alpha,
