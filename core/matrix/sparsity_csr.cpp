...
void SparsityCsr<ValueType, IndexType>::apply_impl(const IMultiVector* b,
                                                   IMultiVector* x) const
{
    apply_mixed_precision_dispatch<ValueType>(
        [this](auto view_b, auto view_x, auto...) {
            this->get_executor()->run(
                sparsity_csr::make_spmv(this, view_b, view_x));
        },
        b, x);
}
...
template <typename ValueType, typename IndexType>
void SparsityCsr<ValueType, IndexType>::apply_impl(const IMultiVector* alpha,
