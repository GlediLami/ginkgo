...
template <typename ValueType>
void PipeCg<ValueType>::apply_impl(const IMultiVector* b, IMultiVector* x) const
{
    ...
    precision_dispatch<ValueType>(
        [this](auto converted_b, auto converted_x) {
            ...
            IMultiVector* rw = this->create_workspace_op_with_type_of(
...
template <typename ValueType>
void PipeCg<ValueType>::apply_impl(const IMultiVector* alpha,
...
template <typename ValueType>
PipeCg<ValueType>::PipeCg(std::shared_ptr<const Executor> exec)
    : EnableLinOp<PipeCg>(std::move(exec))
{}


template <typename ValueType>
PipeCg<ValueType>::PipeCg(const Factory* factory,
