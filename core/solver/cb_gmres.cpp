...
template <typename ValueType>
void CbGmres<ValueType>::apply_impl(const IMultiVector* b,
    ...
    precision_dispatch<ValueType>(
        [this](auto converted_b, auto converted_x) {
            ...
            auto apply_templated = [&](auto value) {
                 ...
                 * - x: selects the krylov vector (which has krylov_dim + 1
                 * vectors)
                 * - y: selects the (row-)element of said krylov vector
                 * - z: selects which column-element of said krylov vector
                 * should be used
                 */
                const dim<3> krylov_bases_dim{krylov_dim + 1, num_rows,
                                              num_rhs};
                ...
                auto hessenberg = Vector::create(
                    exec, dim<2>{krylov_dim + 1, krylov_dim * num_rhs});
                auto buffer =
                    Vector::create(exec, dim<2>{krylov_dim + 1, num_rhs});
                auto givens_sin =
                    Vector::create(exec, dim<2>{krylov_dim, num_rhs});
                auto givens_cos =
                    Vector::create(exec, dim<2>{krylov_dim, num_rhs});
                auto residual_norm_collection =
                    Vector::create(exec, dim<2>{krylov_dim + 1, num_rhs});
                auto residual_norm =
                    VectorNorms::create(exec, dim<2>{1, num_rhs});
                // 1st row of arnoldi_norm: == eta *
                // norm2(old_next_krylov_basis)
                //                          with eta == 1 / sqrt(2)
                //                          (computed right before updating
                //                          next_krylov_basis)
                // 2nd row of arnoldi_norm: The actual arnoldi norm
                //                          == norm2(next_krylov_basis)
                // 3rd row of arnoldi_norm: the infinity norm of
                // next_krylov_basis
                //                          (ONLY when using a scalar accessor)
                auto arnoldi_norm =
                    VectorNorms::create(exec, dim<2>{3, num_rhs});
                ...
                array<stopping_status> stop_status(this->get_executor(),
                                                   num_rhs);
                // reorth_status and num_reorth are both helper variables for
                // GPU implementations at the moment. num_reorth := Number of
                // vectors which require a re-orthogonalization reorth_status :=
                // stopping status for the re-orthogonalization,
                //                  marking which RHS requires one, and which
                //                  does not
                ...
                exec->run(cb_gmres::make_initialize(
                    ...
                    residual->get_device_view(), givens_sin->get_device_view(),
                    givens_cos->get_device_view(), stop_status, krylov_dim));
                // residual = dense_b
                ...
                exec->run(cb_gmres::make_restart(
                    residual->get_const_device_view(),
                ...
                auto stop_criterion =
                    this->get_stop_criterion_factory()->generate(
                        this->get_system_matrix(),
                        std::shared_ptr<const Vector>(dense_b,
                                                      [](const Vector*) {}),
                ...
                const size_type forced_limit{krylov_dim /
                ...
                while (true) {
                    ...
                    if (forced_iterations < forced_limit &&
                        forced_iterations <
                            total_iter / forced_iteration_fraction) {
                        ...
                        this->template log<log::Logger::iteration_complete>(
                            this, dense_b, dense_x, total_iter, residual.get(),
                            residual_norm.get(), nullptr, &stop_status,
                            all_changed);
                        if (one_changed || all_changed) {
                            host_stop_status = stop_status;
                            bool host_array_changed{false};
                            for (size_type i = 0;
                                 i < host_stop_status.get_size(); ++i) {
                                auto local_status =
                                    host_stop_status.get_data() + i;
                                ...
                                if (local_status->has_converged()) {
                                    ...
                                    if (stop_encountered_rhs.get_data()[i] ||
                                        ...
                                        fully_converged_rhs.get_data()[i] =
                                            true;
                                    } else {
                                        stop_encountered_rhs.get_data()[i] =
                                            true;
                                        local_status->reset();
                            ...
                            if (host_array_changed) {
                                ...
                                // non-convergence reason (like time or
                                // iteration)
                                break;
                            }
                            forced_iterations = 0;
                            ...
                            for (size_type i = 0;
                                 i < stop_encountered_rhs.get_size(); ++i) {
                                stop_encountered_rhs.get_data()[i] = false;
                            }
                        }
                    }
                    ...
                    if (perform_reset || restart_iter == krylov_dim) {
                        ...
                        // residual = dense_b
                        this->get_system_matrix()->apply(neg_one_op, dense_x,
                                                         one_op, residual);
                        // residual = residual - Ax
                        exec->run(cb_gmres::make_restart(
                            residual->get_const_device_view(),
                            ...
                            residual_norm_collection->get_device_view(),
                            arnoldi_norm->get_device_view(), krylov_bases_range,
                            next_krylov_basis->get_device_view(),
                    ...
                    auto buffer_iter =
                        buffer->create_subview(local_span{0, restart_iter + 2},
                    ...
                    exec->run(cb_gmres::make_arnoldi(
                        next_krylov_basis->get_device_view(),
                        givens_sin->get_device_view(),
                        givens_cos->get_device_view(),
                        residual_norm->get_device_view(),
                        residual_norm_collection->get_device_view(),
                        krylov_bases_range, hessenberg_iter->get_device_view(),
                        buffer_iter->get_device_view(),
                        arnoldi_norm->get_device_view(), restart_iter,
                ...
                exec->run(cb_gmres::make_solve_krylov(
                    residual_norm_collection->get_const_device_view(),
                    krylov_bases_range.get_accessor().to_const(),
                    hessenberg_small->get_const_device_view(),
                    y->get_device_view(),
            ...
            helper<ValueType>::call(apply_templated,
                                    this->get_storage_precision());
        },
        b, x);
}
...
template <typename ValueType>
void CbGmres<ValueType>::apply_impl(const IMultiVector* alpha,
