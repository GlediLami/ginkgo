...
struct help_compute_norm {
    static void compute_next_krylov_norm_into_hessenberg(
        const IMultiVector* next_krylov,
...
void orthogonalize_mgs(matrix::MultiVector<ValueType>* hessenberg_iter,
                       IMultiVector* krylov_bases, IMultiVector* next_krylov,
...
void finish_reduce(matrix::MultiVector<ValueType>* hessenberg_iter,
                   IMultiVector* next_krylov, const size_type num_rhs,
...
template <typename ValueType>
void Gmres<ValueType>::apply_impl(const IMultiVector* b, IMultiVector* x) const
{
    ...
    IMultiVector* preconditioned_krylov_bases = nullptr;
    if (is_flexible) {
    ...
    while (true) {
        ...
        auto next_krylov = krylov_bases->create_subview(
            ...
            local_span{0, num_rhs}, dim<2>{num_rows, num_rhs});
        std::unique_ptr<IMultiVector> preconditioned_krylov;
...
template <typename ValueType>
void Gmres<ValueType>::apply_impl(const IMultiVector* alpha,
