...
TYPED_TEST(Cg, KernelInitialize)
{
    ...
    gko::kernels::reference::cg::initialize(
        ...
        this->small_rho->get_device_view(), this->small_stop);
...
TYPED_TEST(Cg, ThrowsOnStencilSystemMultiVectorMixed)
{
    using value_type = typename TestFixture::value_type;
    ...
    EXPECT_THROW(solver->apply_mv(b, x), gko::InvalidStateError);
}
