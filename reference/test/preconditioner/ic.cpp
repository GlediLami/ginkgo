...
protected:
    ...
    using isai_type = gko::preconditioner::LowerIsai<value_type, index_type>;
    using ic_type = gko::preconditioner::Ic<value_type, index_type>;
    ...
    Ic()
        : exec(gko::ReferenceExecutor::create()),
          ...
          ic_pre_factory(ic_type::build().on(exec)),
          ic_isai_pre_factory(
              ic_type::build().with_l_solver(isai_type::build()).on(exec)),
...
{
    using isai_type = typename TestFixture::isai_type;
    auto precond = this->ic_isai_pre_factory->generate(this->mtx);

    GKO_ASSERT_MTX_NEAR(
        gko::as<isai_type>(precond->get_l_solver())->get_approximate_inverse(),
        this->l_isai_factor, this->tol);
    GKO_ASSERT_MTX_NEAR(
        gko::as<typename isai_type::transposed_type>(precond->get_lh_solver())
...
TYPED_TEST(Ic, SolvesSingleRhsMixed)
{
    ...
    using Vec = gko::matrix::MultiVector<gko::next_precision<T>>;
    const auto b = gko::initialize<Vec>({1.0, 3.0, 6.0}, this->exec);
    auto x = Vec::create(this->exec, gko::dim<2>{3, 1});
    auto preconditioner =
        ic_prec_type::build().on(this->exec)->generate(this->mtx);
