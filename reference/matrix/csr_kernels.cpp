                  ...
                  matrix::view::csr<ValueType, IndexType> c)
{
    ...
    for (const auto row : irange{num_rows}) {
    ...
    for (const auto row : irange{num_rows}) {
        ...
        for (const auto a_nz : irange{a_begin, a_end}) {
            ...
            for (const auto b_nz : irange{b_begin, b_end}) {
                ...
                const auto rel_nz = c_row_lookup[b_col];
                GKO_ASSERT(rel_nz != invalid_index<IndexType>());
                c_vals[c_begin + rel_nz] += valpha * a_val * b_val;
            }
        }
        // Finally add beta * D
        for (const auto d_nz : irange{d_begin, d_end}) {
            ...
            const auto rel_nz = c_row_lookup[d_col];
            GKO_ASSERT(rel_nz != invalid_index<IndexType>());
                    ...
                    matrix::view::csr<ValueType, IndexType> c)
{
    ...
    abstract_spgeam(
        a, b,
        [&](IndexType row) {
            return std::make_pair(c_row_ptrs[row], c_row_ptrs[row + 1]);
        },
        [&](IndexType, IndexType col, ValueType a_val, ValueType b_val,
            std::pair<IndexType, IndexType>& state) {
            GKO_ASSERT(state.first < state.second);
            c_vals[state.first] = valpha * a_val + vbeta * b_val;
            ++state.first;
        },
        [](IndexType, std::pair<IndexType, IndexType>) {});
}
