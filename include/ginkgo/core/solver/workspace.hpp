...
public:
    ...
    IMultiVector* create_or_get_vector(int op_id, CreateOperation create,
    ...
    const IMultiVector* get_vector(int op_id) const
    {
        GKO_ASSERT(op_id >= 0 && op_id < vectors_.size());
...
private:
    ...
    std::vector<std::unique_ptr<IMultiVector>> vectors_;
