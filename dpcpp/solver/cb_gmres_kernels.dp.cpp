             ...
             size_type krylov_dim)
{
    constexpr bool use_scalar =
        gko::cb_gmres::detail::has_3d_scaled_accessor<Accessor3d>::value;
    const auto num_rows = residual.size[0];
    const auto num_rhs = residual.size[1];
    const auto krylov_stride =
        gko::cb_gmres::helper_functions_accessor<Accessor3d>::get_stride(
            krylov_bases);
    ...
    const dim3 block_dim(default_block_size, 1, 1);
    constexpr auto block_size = default_block_size;
    const auto stride_arnoldi = arnoldi_norm.stride;
...
void finish_arnoldi_CGS(
    ...
    matrix::view::dense<ValueType> next_krylov_basis, Accessor3dim krylov_bases,
    ...
    array<size_type>& num_reorth)
{
    ...
    const auto stride_next_krylov = next_krylov_basis.stride;
    const auto stride_hessenberg = hessenberg_iter.stride;
    const auto stride_buffer = buffer_iter.stride;
    const auto stride_arnoldi = arnoldi_norm.stride;
    ...
    const array<stopping_status>& stop_status)
{
    ...
    constexpr auto block_size = default_block_size;
    const auto num_cols = hessenberg_iter.size[1];
    ...
    matrix::view::dense<ValueType> y, const array<size_type>& final_iter_nums)
{
    // TODO: tune block_size for optimal performance
    constexpr auto block_size = default_block_size;
    const auto num_rhs = residual_norm_collection.size[1];
    const dim3 block_dim{block_size, 1, 1};
                  ...
                  const array<size_type>& final_iter_nums)
{
    ...
    const auto stride_before_preconditioner = before_preconditioner.stride;
                  ...
                  const array<size_type>& final_iter_nums)
{
    ...
    const auto iters = hessenberg.size[1] / before_preconditioner.size[1];
